...
elasticsearch.prototype.addAuth = function(requestObject) {

	if (this.authType !== "aws") {
		// auth added on baseRequest
	...
	aws4.sign(options, {
		accessKeyId: this.config.aws_accessKeyId,
		secretAccessKey: this.config.aws_secretAccessKey
	});

	// add aws auth headers
	requestObject.headers = options.headers;
...
elasticsearch.prototype.testConnection = function (callback) {
	...
	var self = this;
	this.baseRequest(this.addAuth(options), function (error, response, body) {
...
elasticsearch.prototype.deleteIndex = function (index, callback) {
	...
	var options = { method: "DELETE", url: index };
